...
SELECT e.*,
 ...
 /* Создание новой таблицы с клиентами */
CREATE TABLE clients (
id INTEGER PRIMARY KEY AUTOINCREMENT,
name VARCHAR(150),
birth_date DATE);

/*Соединение данных в вертикальной плоскости из двух таблиц*/
SELECT full_name, birth_date FROM employees
UNION ALL
SELECT full_name, birth_date FROM clients;
...
SELECT e.full_name, group_concat(f.title) as favorite_films FROM employees as e
INNER JOIN employee_favorite_films as eff ON eff.employee_id = e.id
INNER JOIN films as f ON eff.film_id = f.f_id
GROUP BY e.id
